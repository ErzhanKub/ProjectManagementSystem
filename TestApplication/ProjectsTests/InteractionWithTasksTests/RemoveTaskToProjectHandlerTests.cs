...
namespace TestApplication.TestProject.InteractionWithTask
{
    public class RemoveTaskToProjectHandlerTests
    {
        ...
        public async Task Handle_ValidCommand_ReturnsOk()
        {
            ...
            var handler = new RemoveTaskToProjectHandler(_projectRepositoryMock.Object, _customTaskRepositoryMock.Object, _unitOfWorkMock.Object);
            var command = new RemoveTaskToProjectCommand { ProjectId = project.Id, TaskId = task.Id };
            // Act
            var result = await handler.Handle(command, default);

            // Assert
            result.IsSuccess.Should().BeTrue();
            project.Tasks.Should().NotContain(task);
        }
    }
}